...
SELECT io.education_label, count(*)as num
FROM import_organisation io 
GROUP BY io.education_label 
ORDER BY num DESC;
...
ALTER TABLE import_organisation ADD COLUMN standard_label TEXT;

-- trim and clean up data
UPDATE import_organisation set standard_label = replace(trim(lower(education_label)), ' ', '-');


-- count again
SELECT io.standard_label, count(*)as num
FROM import_organisation io 
GROUP BY io.standard_label 
ORDER BY num DESC;
...
RENAME COLUMN education_place_1 TO education_place;

ALTER TABLE person_features
RENAME COLUMN education_place_2 TO education_place_uri;

-- not this
SELECT ipe.person_uri
FROM import_organisation io
WHERE io.organisation_uri = ipe.employer_uri 
ORDER BY ioc.coded_class ASC
LIMIT 50;
...
SET education_place_1 = (
    SELECT io.education_label
    FROM import_organisation AS io
    JOIN import_organisation io ON io.organisation_uri = ipe.employer_uri 
...
-- input education_uri and education_label into table person_features
UPDATE person_features
SET 
    education_place = (
        SELECT io.education_label
...
select *
from person_features 
WHERE education_place != 'NA'
AND education_place_uri is not NULL
limit 10;
...
AND education_place is not NULL;
...
SELECT pk_person_features, person_uri, occupation_main, occupation_sec1, education_place
from person_features 
WHERE occupation_sec1 != 'NA'
AND occupation_main != 'NA'
AND education_place is not NULL;
