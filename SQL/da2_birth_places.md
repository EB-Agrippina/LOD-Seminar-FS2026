...
SELECT DISTINCT (?archaeologist AS ?person_uri) ?birth_place_uri (MIN(?birth_place_label) as ?place_label) 
WHERE {
       ...
       ?archaeologist wdt:P19 ?birth_place_uri.
        ...
        GROUP BY ?archaeologist ?birth_place_uri
```
...
SELECT DISTINCT (?archaeologist AS ?person_uri) ?birth_place_uri (MIN(?person_name_al) as ?place_label) 
WHERE {
            {?archaeologist wdt:P106 wd:Q3621491} # archaeologist
            UNION
            {?archaeologist wdt:P101 wd:Q23498} # archaeology field 
